...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      try {
        // 1. handle OAuth redirect tokens (if present in URL)
        if (typeof window !== 'undefined') {
        ...
        if (session?.user) {
          ...
          // 3. fallback: use cached user if we have one
          try {
  ...
  // ---------- HABITS & CHAPTERS (same as before) ----------
